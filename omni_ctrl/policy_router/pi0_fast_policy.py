...
class Pi0FastDroidPolicy(BasePolicy):
    """Wrapper for Pi0-FAST DROID VLA model."""
    ...
    def __init__(
        ...
        policy_type: str = "pi0fast",
        pred_step: int = 5,
        policy_skip_step: int = 3,
        gripper_max: float = 0.7,
    ):
        """Initialize Pi0-FAST DROID policy.

        Args:
            checkpoint_path: Path to Pi0-FAST checkpoint
        ...
        if openpi_path.exists():
            ...
            print("Pi0-FAST policy may not work correctly without openpi module")
        ...
        try:
            ...
            print(f"✓ Pi0-FAST policy loaded from {checkpoint_path}")
            ...
            raise RuntimeError(f"Failed to initialize Pi0-FAST policy: {e}")
    ...
    def predict_chunk(self, obs: Dict, task_instruction: str, chunk_size: int = 10) -> np.ndarray:
        """Predict a full action chunk from Pi0-FAST.

        Uses the same preprocessing as the DROID rollout pipeline:
        - Images are assumed pre-resized to (180, 320) by the orchestrator.
        - `resize_with_pad` is applied to 224x224 for model input.
        """
        try:
            ...
            print(f"Error in Pi0-FAST forward pass: {e}")
            import traceback
            traceback.print_exc()
