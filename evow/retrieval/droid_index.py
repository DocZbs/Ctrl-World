...
class DROIDScenario:
    ...
    Attributes:
        ...
        initial_joints: Initial joint positions (8,)
    ...
    def _build_scenario(self, meta: dict) -> DROIDScenario:
        ...
        if parquet_path is not None:
            ...
            if parquet_joints is not None:
                initial_joints = parquet_joints[:8]
        ...
        if initial_joints is None:
            ...
            if joint_pos and len(joint_pos) > 0:
                ...
                if len(initial_joints) < 8:
                    initial_joints = np.pad(initial_joints, (0, 8 - len(initial_joints)))
                elif len(initial_joints) > 8:
                    initial_joints = initial_joints[:8]
            else:
                initial_joints = np.array([0.0] * 8, dtype=np.float32)
