"""EvoW orchestrator: closed-loop task-to-policy self-evolution (Algorithm 1).
...
from ..evaluation import create_vlm_evaluator
from ..policy_router.policy_router import PolicyRouter
from ..policy_router.vlm_router import VLMPolicyRouter
from ..utils.runtime import PROJECT_ROOT, ensure_openpi_client_on_path, ensure_project_root_on_path
from .episode import Episode
from .skill_library import SkillLibrary
from .evow_memory import EvoWMemory, MemoryEntry
...
class EvoWOrchestrator:
    """EvoW self-evolution loop implementing Algorithm 1.

    Coordinates all EvoW modules in a closed-loop:
    for each task l:
    ...
    def __init__(self, config):
        print("=" * 60)
        print("Initializing EvoW Orchestrator")
        print("=" * 60)
        ...
        if getattr(config, "fixed_scenario", None) and config.fixed_scenario is not None:
            if getattr(config.fixed_scenario, "enabled", False):
                raise ValueError("fixed_scenario is not supported in EvoW orchestrator")
        ...
        self.evaluator = create_vlm_evaluator(config.evaluation, allow_deferred=True)

        # --- EvoW memory ---
        print("\n[7/7] Initializing EvoW memory...")
        self.memory = EvoWMemory(
        ...
        print("\n" + "=" * 60)
        print("EvoW initialization complete!")
        print("=" * 60 + "\n")
    ...
    def run(self):
        """Run EvoW Algorithm 1: closed-loop self-evolution."""
        print(f"Starting EvoW evolution for {self.config.num_iterations} iterations...")
        print(f"Output directory: {self.output_dir}\n")

        for iteration in tqdm(range(self.config.num_iterations), desc="EvoW"):
            print(f"\n{'=' * 60}")
            print(f"Iteration {iteration + 1}/{self.config.num_iterations}")
            print(f"{'=' * 60}")

            try:
                self._evow_iteration(iteration)
            except Exception as e:
        ...
        print("\n" + "=" * 60)
        print("EvoW evolution complete!")
        print("=" * 60)
        self._log_stats(self.config.num_iterations)
        self._export_results()

    def _evow_iteration(self, iteration: int):
        """Single EvoW iteration implementing Algorithm 1 inner loop.
    ...
    def _execute_rollout(self, task, scenario, policy) -> Episode:
        ...
        # One interaction predicts `pred_step` frames but only advances timeline by
        # `pred_step-1` (last frame is reused as next-step condition).
        # Therefore rollout interaction stride should follow `step_advance`.
        for interaction_idx, step in enumerate(range(0, max_steps, step_advance)):
